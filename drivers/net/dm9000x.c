...
dm9000_probe(void)
{
	...
	if (id_val == DM9000_ID) {
		printf("test_dm9000 i/o: 0x%x, id: 0x%x \n", CONFIG_DM9000_BASE,
		       id_val);
		return 0;
	} else {
...
{
	int tmo;
	u16 save_mwr, calc_mwr, check_mwr;
	struct board_info *db = &dm9000_info;
	...
	if(length < 14) 
		printf(" tx len error %d \n", length);
	
//	save_mwr = (DM9000_ior(0xfb) << 8) | DM9000_ior(0xfa);
...
//		check_mwr = (DM9000_ior(0xfb) << 8) | DM9000_ior(0xfa);
//		if(calc_mwr != check_mwr)
//		{
//			printf("transmission point error %04x %04x %04x %04x \n", save_mwr, length, calc_mwr, check_mwr);
//			DM9000_iow(0xfb, (save_mwr >> 8) & 0xff);
//			DM9000_iow(0xfa, save_mwr & 0xff);
//		}
//	}while(calc_mwr != check_mwr);
...
static int dm9000_rx(struct eth_device *netdev)
{
	...
	u16 save_mrr, calc_mrr, check_mrr;
	struct board_info *db = &dm9000_info;
...
//		check_mrr = (DM9000_ior(0xf5) << 8) | DM9000_ior(0xf4);
//		if(calc_mrr != check_mrr)
//		{
//			printf("receiving 4 byte error %04x %04x %04 %04x \n", save_mrr, calc_mrr, check_mrr);
//			DM9000_iow(0xf5, (save_mrr >> 8) & 0xff);
//			DM9000_iow(0xf4, save_mrr & 0xff);
//			continue;
//		}
...
//		if(calc_mrr != check_mrr)
//		{
//			printf("receiving point error %04x %04x %04 %04x \n", save_mrr, RxLen, calc_mrr, check_mrr);
//			DM9000_iow(0xf5, (save_mrr >> 8) & 0xff);
//			DM9000_iow(0xf4, save_mrr & 0xff);
//			continue;
//		}

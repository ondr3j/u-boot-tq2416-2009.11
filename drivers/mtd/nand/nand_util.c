			...
			u_char *buffer)
{
	...
	while (left_to_write > 0) {
		...
		unsigned long long n = (unsigned long long) (tLength-left_to_write) * 100;
		...
		if (percent != percent_complete)
		{
			printf("\rWriting data at 0x%lx -- %3d%% complete.", off, percent);
			percent_complete = percent;
		}
	...
	printf("\rWriting data at 0x%lx -- %3d%% complete.\n", off, 100);
...
#ifdef CONFIG_EMBEDSKY_FAT
int nand_write_skip_bad_OEMyaffs(nand_info_t *nand, loff_t offset, size_t *length,u_char *buffer, unsigned long Dir)
{
	int rval = 0, blocksize;
	size_t left_to_write = *length;
	u_char *p_buffer = buffer;
	...
	fatread_len = (dataLength * pageNumber);
	while (left_to_write > 0) {
		size_t block_offset = offset & (nand->erasesize - 1);
		size_t write_size, truncated_write_size;
		...
		p_buffer = buffer;
		Dir = FAT_OEMReadFile(Dir, p_buffer, fatread_len, 0);

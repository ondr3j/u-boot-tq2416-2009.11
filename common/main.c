...
#endif
#define  EMBEDSKY_FATDEBUG   0
#if EMBEDSKY_FATDEBUG
static char testarg1=0;
char testarg2;
void test_hsmmc1(void);
#endif
#include <post.h>
#ifdef CONFIG_S3C2416
#include <s3c2416.h>
#endif
#define rGPA_CON		(*(volatile unsigned *)(0x56000000))
...
#endif
	putc('\n');
	c = awaitkey(0x1, NULL);						//HJ_add start 20091128 判断是否有按键输入
	if(c == 0x20)
	{	/* we got a key press	*/
		abort  = 1;	/* don't auto boot	*/
	}									//HJ_add end 20091128
		...
		embedsky_user_logo();
			...
			printf("SD:File System init failed!\n");
		}
#endif
...
void main_loop (void)
{
	...
	printf("\r\n##### %s BIOS for %s #####\r\n", buf1, buf2);
	printf("This Board: SDRAM is 64MB; ");
#ifdef CONFIG_EmbedSky_LCD
	tq2416_lcd();
	printf("LCD display size is: %d X %d\r\n", SCR_XSIZE_TFT, SCR_YSIZE_TFT);
#else
	printf("\n");
#endif

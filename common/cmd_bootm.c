...
#ifndef CONFIG_BOOTM_LEN
#define CONFIG_BOOTM_LEN	0x800000	/* use 8MByte as default max gunzip size */
#endif
...
ulong load_addr = CONFIG_LOAD_ADDR;	/* Default Load Address */
...
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	run_command("run bootargs_defaults", 0);
...
int do_imls (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	for (i = 0, info = &flash_info[0];
		i < CONFIG_MAX_FLASH_BANKS; ++i, ++info) {
			   ...
			   bootm_headers_t *images)
{
	...
	if (top > CONFIG_BOOTMAPSZ)
		top = CONFIG_BOOTMAPSZ;
	top &= ~0xF;

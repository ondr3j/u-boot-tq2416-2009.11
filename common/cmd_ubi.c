...
static int ubi_volume_OEM_write(char *volume, void *buf, char *filename)
{
...
#ifdef CONFIG_TQ2416
	printf("\nUBIFS image length is 0x%x, Write UBIFS to %s, please wait.\n", size, volume);
#endif
	...
	if (err < 0) {
		printf("Cannot start volume update,err=%d\n",err);
		return err;
	}
...
static int do_ubi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strncmp(argv[1], "write", 5) == 0) {
		if (argc < 5) {
			printf("Please see usage current argc:%d\n",argc);
			printf("%s,%s\n",argv[2],argv[4]);
			return 1;
		}
		...
		printf("write images for add:%x,size:%x\n",addr,size);
